...
def test_workflow_state_payload_persists_echo_heatmap_settings() -> None:
    ...
    window.radar_outlier_removal_enabled_var = SimpleNamespace(get=lambda: True)
    window.radar_outlier_band_width_cm_var = SimpleNamespace(get=lambda: "25")
    window._records = []
    ...
    assert payload["radar_outlier_removal_enabled"] is True
    assert payload["radar_outlier_band_width_cm"] == 25.0


def test_draw_selected_echo_probability_overlay_grays_lidar_line_outliers() -> None:
    class FakeCanvas:
        def __init__(self) -> None:
            self.ovals: list[tuple[tuple[float, ...], dict[str, object]]] = []
    ...
    window.echo_heatmap_imaginary_line_width_var = SimpleNamespace(get=lambda: "4")
    window.radar_outlier_removal_enabled_var = SimpleNamespace(get=lambda: True)
    window.radar_outlier_band_width_cm_var = SimpleNamespace(get=lambda: "20")
    window._append_validation = lambda _message: None
    window._preview_pixel_to_world = lambda *, preview_x, preview_y: (preview_x / 10.0, preview_y / 10.0)
    window._build_echo_overlay_preview_points = lambda **_kwargs: ([10.0, 0.0, 10.0, 10.0], 1)
    records = [
        {
            "live_position_at_measurement": {"x": float(index), "y": 0.0},
            "measurement": {"result": {"echo_delays": [{"distance_m": 1.0}]}},
    ...
    lidar_wall_estimate = LidarWallEstimate(
        slope=0.0,
        intercept=0.0,
        start=(0.0, 0.0),
        end=(2.0, 0.0),
...
def test_set_measurement_map_pick_mode_clears_overlay_when_disabled() -> None:
    ...
    assert window.measurement_map_pick_mode_btn.text == "Lineal"
